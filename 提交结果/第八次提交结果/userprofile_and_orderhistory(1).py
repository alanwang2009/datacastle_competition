...
def up_oh_statistics(up_path,oh_path,outputpath):
    ...
    prev_cus_id='0'
    newCust=pd.DataFrame(columns=["userid",'nearestOrderTime',
    ...
    for i in range(0,len(oh_data)):
        ...
        if uid not in idlist:
            ...
            finalud={'userid':uid,'nearestOrderTime':nearestOd,
    ...
    data_1=up_oh_data[['genderN','provinceN']]
    data_2=pd.get_dummies(data_1)
    result_1=pd.concat([up_oh_data,data_2],axis=1)
    result=result_1[result_1.userid>206]
    result.drop(['genderN','provinceN'], axis=1,inplace=True)
